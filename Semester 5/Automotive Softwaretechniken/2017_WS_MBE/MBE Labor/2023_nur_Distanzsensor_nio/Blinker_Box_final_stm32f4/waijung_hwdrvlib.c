...
void enable_CANReceive(void)
{
  ...
  if (CANReceive_filter_number < CAN_FILTER_COUNT) {
    ...
    CANReceive_FilterInitStructure.CAN_FilterIdHigh = 0x1C0;
...
/* ########################################################################
 * Name: <S1>/CAN Receive2
 * Id: CANReceive2
 * ########################################################################
 */
CAN_FilterInitTypeDef CANReceive2_FilterInitStructure;
uint8_t CANReceive2_filter_number;     /*Filter matched index */
CANReceive2_DATA_STRUCT *CANReceive2_data;
CanRxMsg CANReceive2_Msg;              /* Temporary message */
CanRxMsg CANReceive2_Msg_Buffer[8];    /* Size must be a number of 2^N */
volatile uint16_t CANReceive2_Msg_Buffer_RIndex = 0;
volatile uint16_t CANReceive2_Msg_Buffer_WIndex = 0;
uint16_t CANReceive2_Msg_Buffer_Count(void)
{
  volatile uint16_t read_index;
  volatile uint16_t write_index;

  /* Get index to local variable, due to global will also access by ISR */
  read_index = CANReceive2_Msg_Buffer_RIndex;
  write_index = CANReceive2_Msg_Buffer_WIndex;
  if (write_index >= read_index) {
    return (write_index - read_index);
  } else {
    return ((8-read_index) + write_index);/* Max count is 8-1 */
...
uint16_t CANReceive2_GetMsg(CanRxMsg* pMsg)
{
  uint16_t msg_pending;
  __disable_irq();
  msg_pending = CANReceive2_Msg_Buffer_Count();
  if (msg_pending > 0) {               //CANReceive2_Msg_Buffer_RIndex != CANReceive2_Msg_Buffer_WIndex) {
    memcpy(pMsg, &CANReceive2_Msg_Buffer[CANReceive2_Msg_Buffer_RIndex], sizeof
           (CanRxMsg));
    CANReceive2_Msg_Buffer_RIndex ++;
    CANReceive2_Msg_Buffer_RIndex &= (8-1);
  }
...
void CANReceive2_filter_isr(CanRxMsg* pMsg)
{
  if (pMsg->IDE == CAN_Id_Standard) {  /* Accept only ID Standard */
    memcpy(&CANReceive2_Msg_Buffer[CANReceive2_Msg_Buffer_WIndex] ,pMsg, sizeof
           (CanRxMsg));
    CANReceive2_Msg_Buffer_WIndex ++ ;
    CANReceive2_Msg_Buffer_WIndex &= (8-1);
    if (CANReceive2_Msg_Buffer_WIndex == CANReceive2_Msg_Buffer_RIndex) {/* Buffer overflow, Flush an oldest queue */
      CANReceive2_Msg_Buffer_RIndex ++;
      CANReceive2_Msg_Buffer_RIndex &= (8-1);
    }
  }
...
void enable_CANReceive2(void)
{
  ...
  /* Get available filter number */
  CANReceive2_filter_number = CAN1_Filter_Hook(CANReceive2_filter_isr);
  if (CANReceive2_filter_number < CAN_FILTER_COUNT) {
    /* CAN filter init:
       ID Type: Standard, Filter: Specific
     */
    CANReceive2_FilterInitStructure.CAN_FilterNumber = CANReceive2_filter_number;
    CANReceive2_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
    CANReceive2_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
    CANReceive2_FilterInitStructure.CAN_FilterIdHigh = 0x300;
    CANReceive2_FilterInitStructure.CAN_FilterIdLow = 0x0;
    CANReceive2_FilterInitStructure.CAN_FilterMaskIdHigh = 0xFFE0;
    CANReceive2_FilterInitStructure.CAN_FilterMaskIdLow = 0x4;
    CANReceive2_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
    CANReceive2_FilterInitStructure.CAN_FilterActivation = ENABLE;
    CAN_FilterInit(&CANReceive2_FilterInitStructure);
  } else {
    /* Error: No filter avalable, All (0-13) already acquired */
  }

  /* Initial buffer */
  CANReceive2_Msg_Buffer_RIndex = 0;
  CANReceive2_Msg_Buffer_WIndex = 0;
}

void disable_CANReceive2(void)
{
  /* Disable filter */
  CANReceive2_FilterInitStructure.CAN_FilterActivation = DISABLE;
  CAN_FilterInit(&CANReceive2_FilterInitStructure);

  /* Release hook */
  CAN1_Filter_Free(CANReceive2_filter_number);
...
/* ########################################################################
 * Name: <S1>/CAN Receive3
 * Id: CANReceive3
 * ########################################################################
 */
CAN_FilterInitTypeDef CANReceive3_FilterInitStructure;
uint8_t CANReceive3_filter_number;     /*Filter matched index */
CANReceive3_DATA_STRUCT *CANReceive3_data;
CanRxMsg CANReceive3_Msg;              /* Temporary message */
CanRxMsg CANReceive3_Msg_Buffer[8];    /* Size must be a number of 2^N */
volatile uint16_t CANReceive3_Msg_Buffer_RIndex = 0;
volatile uint16_t CANReceive3_Msg_Buffer_WIndex = 0;
uint16_t CANReceive3_Msg_Buffer_Count(void)
{
  volatile uint16_t read_index;
  volatile uint16_t write_index;

  /* Get index to local variable, due to global will also access by ISR */
  read_index = CANReceive3_Msg_Buffer_RIndex;
  write_index = CANReceive3_Msg_Buffer_WIndex;
  if (write_index >= read_index) {
    return (write_index - read_index);
  } else {
    return ((8-read_index) + write_index);/* Max count is 8-1 */
...
uint16_t CANReceive3_GetMsg(CanRxMsg* pMsg)
{
  uint16_t msg_pending;
  __disable_irq();
  msg_pending = CANReceive3_Msg_Buffer_Count();
  if (msg_pending > 0) {               //CANReceive3_Msg_Buffer_RIndex != CANReceive3_Msg_Buffer_WIndex) {
    memcpy(pMsg, &CANReceive3_Msg_Buffer[CANReceive3_Msg_Buffer_RIndex], sizeof
           (CanRxMsg));
    CANReceive3_Msg_Buffer_RIndex ++;
    CANReceive3_Msg_Buffer_RIndex &= (8-1);
  }
...
void CANReceive3_filter_isr(CanRxMsg* pMsg)
{
  if (pMsg->IDE == CAN_Id_Standard) {  /* Accept only ID Standard */
    memcpy(&CANReceive3_Msg_Buffer[CANReceive3_Msg_Buffer_WIndex] ,pMsg, sizeof
           (CanRxMsg));
    CANReceive3_Msg_Buffer_WIndex ++ ;
    CANReceive3_Msg_Buffer_WIndex &= (8-1);
    if (CANReceive3_Msg_Buffer_WIndex == CANReceive3_Msg_Buffer_RIndex) {/* Buffer overflow, Flush an oldest queue */
      CANReceive3_Msg_Buffer_RIndex ++;
      CANReceive3_Msg_Buffer_RIndex &= (8-1);
    }
  }
...
void enable_CANReceive3(void)
{
  ...
  /* Get available filter number */
  CANReceive3_filter_number = CAN1_Filter_Hook(CANReceive3_filter_isr);
  if (CANReceive3_filter_number < CAN_FILTER_COUNT) {
    /* CAN filter init:
       ID Type: Standard, Filter: Specific
     */
    CANReceive3_FilterInitStructure.CAN_FilterNumber = CANReceive3_filter_number;
    CANReceive3_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
    CANReceive3_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
    CANReceive3_FilterInitStructure.CAN_FilterIdHigh = 0x400;
    CANReceive3_FilterInitStructure.CAN_FilterIdLow = 0x0;
    CANReceive3_FilterInitStructure.CAN_FilterMaskIdHigh = 0xFFE0;
    CANReceive3_FilterInitStructure.CAN_FilterMaskIdLow = 0x4;
    CANReceive3_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
    CANReceive3_FilterInitStructure.CAN_FilterActivation = ENABLE;
    CAN_FilterInit(&CANReceive3_FilterInitStructure);
  } else {
    /* Error: No filter avalable, All (0-13) already acquired */
  }

  /* Initial buffer */
  CANReceive3_Msg_Buffer_RIndex = 0;
  CANReceive3_Msg_Buffer_WIndex = 0;
}

void disable_CANReceive3(void)
{
  /* Disable filter */
  CANReceive3_FilterInitStructure.CAN_FilterActivation = DISABLE;
  CAN_FilterInit(&CANReceive3_FilterInitStructure);

  /* Release hook */
  CAN1_Filter_Free(CANReceive3_filter_number);
...
/* ########################################################################
 * Name: <S1>/CAN Receive4
 * Id: CANReceive4
 * ########################################################################
 */
CAN_FilterInitTypeDef CANReceive4_FilterInitStructure;
uint8_t CANReceive4_filter_number;     /*Filter matched index */
CANReceive4_DATA_STRUCT *CANReceive4_data;
CanRxMsg CANReceive4_Msg;              /* Temporary message */
CanRxMsg CANReceive4_Msg_Buffer[8];    /* Size must be a number of 2^N */
volatile uint16_t CANReceive4_Msg_Buffer_RIndex = 0;
volatile uint16_t CANReceive4_Msg_Buffer_WIndex = 0;
uint16_t CANReceive4_Msg_Buffer_Count(void)
{
  volatile uint16_t read_index;
  volatile uint16_t write_index;

  /* Get index to local variable, due to global will also access by ISR */
  read_index = CANReceive4_Msg_Buffer_RIndex;
  write_index = CANReceive4_Msg_Buffer_WIndex;
  if (write_index >= read_index) {
    return (write_index - read_index);
  } else {
    return ((8-read_index) + write_index);/* Max count is 8-1 */
...
uint16_t CANReceive4_GetMsg(CanRxMsg* pMsg)
{
  uint16_t msg_pending;
  __disable_irq();
  msg_pending = CANReceive4_Msg_Buffer_Count();
  if (msg_pending > 0) {               //CANReceive4_Msg_Buffer_RIndex != CANReceive4_Msg_Buffer_WIndex) {
    memcpy(pMsg, &CANReceive4_Msg_Buffer[CANReceive4_Msg_Buffer_RIndex], sizeof
           (CanRxMsg));
    CANReceive4_Msg_Buffer_RIndex ++;
    CANReceive4_Msg_Buffer_RIndex &= (8-1);
  }
...
void CANReceive4_filter_isr(CanRxMsg* pMsg)
{
  if (pMsg->IDE == CAN_Id_Standard) {  /* Accept only ID Standard */
    memcpy(&CANReceive4_Msg_Buffer[CANReceive4_Msg_Buffer_WIndex] ,pMsg, sizeof
           (CanRxMsg));
    CANReceive4_Msg_Buffer_WIndex ++ ;
    CANReceive4_Msg_Buffer_WIndex &= (8-1);
    if (CANReceive4_Msg_Buffer_WIndex == CANReceive4_Msg_Buffer_RIndex) {/* Buffer overflow, Flush an oldest queue */
      CANReceive4_Msg_Buffer_RIndex ++;
      CANReceive4_Msg_Buffer_RIndex &= (8-1);
    }
  }
...
void enable_CANReceive4(void)
{
  ...
  /* Get available filter number */
  CANReceive4_filter_number = CAN1_Filter_Hook(CANReceive4_filter_isr);
  if (CANReceive4_filter_number < CAN_FILTER_COUNT) {
    /* CAN filter init:
       ID Type: Standard, Filter: Specific
     */
    CANReceive4_FilterInitStructure.CAN_FilterNumber = CANReceive4_filter_number;
    CANReceive4_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;
    CANReceive4_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;
    CANReceive4_FilterInitStructure.CAN_FilterIdHigh = 0x140;
    CANReceive4_FilterInitStructure.CAN_FilterIdLow = 0x0;
    CANReceive4_FilterInitStructure.CAN_FilterMaskIdHigh = 0xFFE0;
    CANReceive4_FilterInitStructure.CAN_FilterMaskIdLow = 0x4;
    CANReceive4_FilterInitStructure.CAN_FilterFIFOAssignment = 0;
    CANReceive4_FilterInitStructure.CAN_FilterActivation = ENABLE;
    CAN_FilterInit(&CANReceive4_FilterInitStructure);
  } else {
    /* Error: No filter avalable, All (0-13) already acquired */
  }

  /* Initial buffer */
  CANReceive4_Msg_Buffer_RIndex = 0;
  CANReceive4_Msg_Buffer_WIndex = 0;
}

void disable_CANReceive4(void)
{
  /* Disable filter */
  CANReceive4_FilterInitStructure.CAN_FilterActivation = DISABLE;
  CAN_FilterInit(&CANReceive4_FilterInitStructure);

  /* Release hook */
  CAN1_Filter_Free(CANReceive4_filter_number);
 ...
 * Id: OutputDigitalOutput
 * ########################################################################
 */
void enable_OutputDigitalOutput(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ENABLE);

  /* Configure PC in output Push Pull mode for OutputDigitalOutput Block*/
...
UART_TX_STATE SetupUARTSetup_Tx_State;
...
/* DMA Init struct */
DMA_InitTypeDef SetupUARTSetup_DMA_Init;
...
{
  if (SetupUARTSetup_Tx_State != txIdle) {
    ...
    /* Update state */
    SetupUARTSetup_Tx_State = txIdle;
  }

  return SetupUARTSetup_Tx_State;
}
...
void UART2_TxUpdate(uint16_t count)
{
  if (count > 0) {
    /* Wait for Tx state to idle */
    UART2_FlushTxBuffer();
    SetupUARTSetup_DMA_Init.DMA_BufferSize = (uint16_t)count;
    DMA_Init(DMA1_Stream6, &SetupUARTSetup_DMA_Init);
    ...
    /* Tx is busy */
    SetupUARTSetup_Tx_State = txBusy;
  }
}
...
void waijung_usart2_initial(void)
{
  ...
  if (!waijung_usart2_ready) {
    ...
    SetupUARTSetup_DMA_Init.DMA_PeripheralBaseAddr = (uint32_t)(&USART2->DR);
    SetupUARTSetup_DMA_Init.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    SetupUARTSetup_DMA_Init.DMA_MemoryInc = DMA_MemoryInc_Enable;
    SetupUARTSetup_DMA_Init.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
    SetupUARTSetup_DMA_Init.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
    SetupUARTSetup_DMA_Init.DMA_FIFOMode = DMA_FIFOMode_Disable;
    SetupUARTSetup_DMA_Init.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;
    SetupUARTSetup_DMA_Init.DMA_MemoryBurst = DMA_MemoryBurst_Single;
    SetupUARTSetup_DMA_Init.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;

    /* Configure DMA controller to manage RX DMA request */
    SetupUARTSetup_DMA_Init.DMA_Channel = DMA_Channel_4;
    SetupUARTSetup_DMA_Init.DMA_Memory0BaseAddr = (uint32_t)UART2_Rx_Buffer;
    SetupUARTSetup_DMA_Init.DMA_DIR = DMA_DIR_PeripheralToMemory;
    SetupUARTSetup_DMA_Init.DMA_BufferSize = (uint16_t)URX2_BUFFER_SIZE;
    SetupUARTSetup_DMA_Init.DMA_Mode = DMA_Mode_Circular;
    SetupUARTSetup_DMA_Init.DMA_Priority = DMA_Priority_VeryHigh;
    DMA_Init(DMA1_Stream5, &SetupUARTSetup_DMA_Init);
    ...
    SetupUARTSetup_DMA_Init.DMA_Channel = DMA_Channel_4;
    SetupUARTSetup_DMA_Init.DMA_Memory0BaseAddr = (uint32_t)UART2_Tx_Buffer;
    SetupUARTSetup_DMA_Init.DMA_DIR = DMA_DIR_MemoryToPeripheral;
    SetupUARTSetup_DMA_Init.DMA_BufferSize = (uint16_t)UTX2_BUFFER_SIZE;
    SetupUARTSetup_DMA_Init.DMA_Mode = DMA_Mode_Normal;
    SetupUARTSetup_DMA_Init.DMA_Priority = DMA_Priority_High;
...
void enable_SetupUARTSetup(void)
{
  WAIJUNG_USARt2_INIT();
}
